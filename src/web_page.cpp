		...
		else
		{
			...
			while (begin && encoding.empty()) {
				...
				if (s1 && s1 < end) {
					...
					if (s1) {
						s1 += strlen("charset");
						while (!isalnum(s1[0]) && s1 < end) { s1++; }
						while ((isalnum(s1[0]) || s1[0] == '-') && s1 < end) { encoding += s1[0]; s1++; }
					}
				}
				if (encoding.empty()) {
...
void web_file::OnFinish(DWORD dwError, LPCWSTR errMsg) {
	...
	if (dwError) {
		...
		switch (m_type) {
		case web_file_document: m_page->on_document_error(dwError, errMsg); break;
		case web_file_waited: m_page->on_waited_finished(dwError, m_file); break;
		}
	}
	else {
		switch (m_type) {
		case web_file_document: m_page->on_document_loaded(m_file, m_encoding.empty() ? NULL : m_encoding.c_str(), m_realUrl.empty() ? NULL : m_realUrl.c_str()); break;
		case web_file_image_redraw: m_page->on_image_loaded(m_file, m_url.c_str(), true); break;
		case web_file_image_rerender: m_page->on_image_loaded(m_file, m_url.c_str(), false); break;
		case web_file_waited: m_page->on_waited_finished(dwError, m_file); break;
		}
	}
}
